...
3. **Local pointer files** — each repository's `AGENTS.md` (with the `CLAUDE.md` that imports it, and any optional adapter a runtime genuinely needs), pointing to the canonical docs and memory root indexes and adding only repo-specific nuance and genuinely tool-specific settings.
