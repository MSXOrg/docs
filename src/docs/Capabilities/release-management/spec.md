...
- **Stable and prerelease.** Every release is either **stable** (the latest version to adopt) or a **prerelease** (testable, not promoted to latest). A prerelease MUST be obtainable either from an open pull request carrying `release:prerelease`, using its explicit owned bump label or the resolved `DefaultBump`, or from a prerelease branch.
