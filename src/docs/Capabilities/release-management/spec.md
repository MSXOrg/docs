...
- An open pull request carrying `release:pre-release` publishes a prerelease without promoting it to latest, using one explicit bump label when present and the resolved `DefaultBump` otherwise.
