...
- **Segmentation before loading.** An agent MUST segment work by host, organization, repository, path, and task before loading project standards. User-global instructions and the active repository context supply the coordinates that make this possible; a per-repository router MUST NOT restate them.
