...
- **Predictable project context.** User-global instructions MUST identify the canonical documentation repository and its local clone path for each adopting organization.
...
- **Segmentation before loading.** An agent MUST segment work by host, organization, repository, path, and task before loading project standards. The repository router MUST supply the coordinates that make this possible by naming its host and organization. The instruction to segment belongs to the user-global instructions, which run before any repository file is read; a per-repository file MUST NOT restate it.
