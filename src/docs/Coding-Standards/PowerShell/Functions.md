...
Every public function in a module built with the PSModule framework uses its generated online reference URL as the first `.LINK` entry. The canonical URL is an absolute HTTPS URL on the module's published documentation host and always ends in `/`: `https://<published-documentation-host>/<ModuleName>/Functions/<Function-Name>/` or, for a grouped command, `https://<published-documentation-host>/<ModuleName>/Functions/<Group>/<Function-Name>/`. A private helper links to the published public function it supports.
